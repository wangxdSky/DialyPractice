...
public class HorizontalRefreshLayout extends FrameLayout {
    ...
    private boolean isOnIntercept = true; //是否进行拦截
    ...
    public boolean isIsOnIntercept() {
        return isOnIntercept;
    }

    public void setIsOnIntercept(boolean isOnIntercept) {
        this.isOnIntercept = isOnIntercept;
    }
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (Math.abs(deltaX) > Math.abs(deltaY)) {//判断是否是水平滑动
                    if (leftHeaderView != null && deltaX > 0 && !canChildScrollRight() && refreshState != REFRESH_STATE_REFRESHING) {//手指向右滑动
                        ...
                        return isOnIntercept ? true : false;
    ...
    private void smoothRelease() {
        ...
        if (refreshCallback != null) {
            refreshCallback.onRefreshRelease();
        }
    }
